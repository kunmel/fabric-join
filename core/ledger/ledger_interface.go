...
import (
	...
	//	"github.com/hyperledger/fabric/core/ledger/kvledger/txmgmt/statedb"  //import cycle
...
type PeerLedger interface {
	...
	// NEW add
//	CrossRollbackOrigVal(kov *[]statedb.KeyOrigVal)  //import statedb导致import cycle // 实现在core/ledger/kvledger/kv_ledger.go
	//CrossRollbackOrigVal(kov *[]cross.KeyOrigVal)
}
...
type QueryExecutor interface {
	//NEW add 2 lines
	// GetStateNoRSet gets the value for given namespace and key. For a chaincode, the namespace corresponds to the chaincodeId,不写入read-set
	// GetStateNoRSet(namespace string, key string) ([]byte, error)

//	GetStateNoRSet(namespace string, key string) (*statedb.VersionedValue, error)  // statedb 导致 import cycle
	GetStateNoRSet(namespace string, key string) ([]byte, error)  //NEW add
