...
//NEW add print
fmt.Println("这里是fabric/core/ledger/kvledger/txmgmt/txmgr/lockbasedtxmgr/lockbased_query_executer.go GetState()")
	return q.helper.getState(ns, key)
}

// NEW add
// NEW GetStateNoRSet implements method in interface `ledger.QueryExecutor`
/*func (q *lockBasedQueryExecutor) GetStateNoRSet(ns string, key string) ([]byte, error) {
//NEW add print
fmt.Println("这里是fabric/core/ledger/kvledger/txmgmt/txmgr/lockbasedtxmgr/lockbased_query_executer.go GetState()")
	return q.helper.getStateNoRSet(ns, key)
}*/
//NEW add
/*func (q *lockBasedQueryExecutor) GetStateNoRSet(ns string, key string) (*statedb.VersionedValue, error) {
	//NEW add print
	fmt.Println("这里是fabric/core/ledger/kvledger/txmgmt/txmgr/lockbasedtxmgr/lockbased_query_executer.go GetState()")
	return q.helper.getStateNoRSet(ns, key) //如果没找到返回nil, nil
}*/
func (q *lockBasedQueryExecutor) GetStateNoRSet(ns string, key string) ([]byte, error) {  //NEW add update
	//NEW add print
	fmt.Println("这里是fabric/core/ledger/kvledger/txmgmt/txmgr/lockbasedtxmgr/lockbased_query_executer.go GetState()")
	return q.helper.getStateNoRSet(ns, key) //如果没找到返回nil, nil
}
...
// NEW add print
fmt.Println("这里是fabric/core/ledger/kvledger/txmgmt/txmgr/lockbasedtxmgr/lockbased_query_executer.go ExecuteQuery()")
