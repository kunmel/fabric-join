...
// NEW add print
fmt.Println("这里是fabric/core/ledger/kvledger/txmgmt/txmgr/lockbasedtxmgr/helper.go getState()")
...
// NEW add 此处read不记录读集 （用来读origVal时用）
/*func (h *queryHelper) getStateNoRSet(ns string, key string) ([]byte, error) {
	fmt.Println("这里是fabric/core/ledger/kvledger/txmgmt/txmgr/lockbasedtxmgr/helper.go getStateNoRSet()")
	if err := h.checkDone(); err != nil {
		return nil, err
	}
	versionedValue, err := h.txmgr.db.GetState(ns, key)
	...
	val, _ := decomposeVersionedValue(versionedValue)
...
func (h *queryHelper) getStateNoRSet(ns string, key string) ([]byte, error) {
	...
	dbVal, err := h.txmgr.db.GetStateByte(ns, key) //如果get的key没有 返回的是nil, nil
	if err != nil {
		return nil, err
	}
...
// NEW add
/*func (h *queryHelper) getStateNoRSet(ns string, key string) (*statedb.VersionedValue, error) {
	fmt.Println("这里是fabric/core/ledger/kvledger/txmgmt/txmgr/lockbasedtxmgr/helper.go getStateNoRSet()")
	if err := h.checkDone(); err != nil {
		return nil, err
	}
	versionedValue, err := h.txmgr.db.GetState(ns, key)  //如果get的key没有 返回的是nil, nil
	if err != nil {
		return nil, err
	}
