...
// 
// NEW add
func (vdb *versionedDB) GetConstructLevelKey(namespace string, key string) []byte {
	compositeKey := constructCompositeKey(namespace, key)
	return vdb.db.GetConstructLevelKey(compositeKey)
}
// NEW add
func (vdb *versionedDB) GetStateByte(namespace string, key string) ([]byte, error){
...
func (vdb *versionedDB) ApplyUpdates(batch *statedb.UpdateBatch, height *version.Height) error {
	fmt.Println("这里是 core/ledger/kvledger/txmgmt/statedb/dtateleveldb/stateleveldb.go") //NEW add

	dbBatch := leveldbhelper.NewUpdateBatch() // 一个空map
	namespaces := batch.GetUpdatedNamespaces() // batch里的ns
	for _, ns := range namespaces {
		updates := batch.GetUpdates(ns)
		for k, vv := range updates {
			compositeKey := constructCompositeKey(ns, k) // ns: ccName
			fmt.Println("ns = ", ns) // NEW add
			fmt.Println("k = ", k) // NEW add
			fmt.Println("compositeKey = ", compositeKey) // NEW add
			fmt.Println("vv = ", vv) //NEW add
...
// NEW add
/*func (vdb *versionedDB) ApplyCrossOrigVal(kov *[]statedb.KeyOrigVal) {
	dbBatch := leveldbhelper.NewUpdateBatch() // 一个空map
	for _, v := range *kov{
		compositeKey := constructCompositeKey(v.Namespace, v.Key) // ns: ccName
		dbBatch.Put(compositeKey, EncodeValue(v.OriginalVersionedValue.Value, v.OriginalVersionedValue.Version))
	}
	vdb.db.WriteBatch(dbBatch, true)
}*/
//NEW add
func (vdb *versionedDB) ApplyCrossOrigVal(kov *[]statedb.KeyOrigVal) {
	dbBatch := leveldbhelper.NewUpdateBatch() // 一个空map
	for _, v := range *kov{
		compositeKey := constructCompositeKey(v.Namespace, v.Key) // ns: ccName
		if v.OriginalVersionedValue == nil{
			dbBatch.Delete(compositeKey)
			continue
		}
		dbBatch.Put(compositeKey, v.OriginalVersionedValue)
	}
	vdb.db.WriteBatch(dbBatch, true)
}
