...
import (
	...
	"github.com/hyperledger/fabric/core/ledger/kvledger/crossdb/crossleveldb"
...
type Provider struct {
	...
	crossdbProvider		crossdb.CrossDBProvider //NEW add
}
...
func NewProvider() (ledger.PeerLedgerProvider, error) {
	...
	crossdbProvider := crossleveldb.NewCrossDBProvider() //NEW add
	...
	provider := &Provider{idStore, ledgerStoreProvider, vdbProvider, historydbProvider, configHistoryMgr, nil, bookkeepingProvider, crossdbProvider}
...
func (provider *Provider) Open(ledgerID string) (ledger.PeerLedger, error) {
	fmt.Println("这里是core/ledger/kvledger/kv_ledger_provider.go  Open()") //NEW add
	fmt.Println("Open(ledgerID), ledgerID = ", ledgerID) // NEW add
...
func (provider *Provider) openInternal(ledgerID string) (ledger.PeerLedger, error) {
	...
	crossDB, err := provider.crossdbProvider.GetDBHandler(ledgerID)
	if err != nil {
		return nil, err
	}
	...
	//l, err := newKVLedger(ledgerID, blockStore, vDB, historyDB, provider.configHistoryMgr, provider.stateListeners, provider.bookkeepingProvider)  // ！！！！这里
	// NEW update
	l, err := newKVLedger(ledgerID, blockStore, vDB, historyDB, crossDB, provider.configHistoryMgr, provider.stateListeners, provider.bookkeepingProvider)
	if err != nil {
		return nil, err
	}
	return l, nil
