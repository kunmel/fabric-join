...
func CreateLedger(genesisBlock *common.Block) (ledger.PeerLedger, error) {
	...
	fmt.Println("这里是 core/ledger/ledgermgmt/ledger_mgmt.go  func CreateLedger()")
	fmt.Println(id, l)   // NEW add
...
func OpenLedger(id string) (ledger.PeerLedger, error) {
	...
	fmt.Println("这里是 core/ledger/ledgermgmt/ledger_mgmt.go  func OpenLedger() id = ", id) // NEW add
...
// NEW add
func GetLedger(id string) (ledger.PeerLedger, error) {
	l, ok := openedLedgers[id]
	if ok {
		return l, nil
	}
