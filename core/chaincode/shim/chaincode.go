		...
		for {
			...
			if recv {
				...
				go func() {
					...
					in2, err = stream.Recv()  //等待接收peer侧链码消息
					errc <- err
					msgAvail <- in2 //发送到msgAvail消息通道
				}()
			}
			select {
			case sendErr := <-errc:
			...
			err = handler.handleMessage(in, errc)  // in 从peer传过来的消息 即core/chaincode/handler.go HandleTransaction()中的resp
			if err != nil {
