...
func (cs *ChaincodeSupport) Execute(ctxt context.Context, cccid *ccprovider.CCContext, spec ccprovider.ChaincodeSpecGetter) (*pb.Response, *pb.ChaincodeEvent, error) {
	resp, err := cs.Invoke(ctxt, cccid, spec)  //返回ChaincodeMessage
	if err != nil {
...
/*	case pb.ChaincodeMessage_CROSS_KEY_LOCKED:  //NEW add
		...
		res.Status = 501 //此处501我们把他当成CrossLocked  在core/endorser/endorser.go ProcessProposal()中判断
		return res, resp.ChaincodeEvent, nil  //New add end*/
