...
import (
	"fmt"
	"github.com/hyperledger/fabric/core/ledger/kvledger"  // NEW add
	"net"
	"runtime"
	"sync"
...
type chainSupport struct {
	...
	rollbackInterface kvledger.RollbackInterface  // NEW add
}
...
func Initialize(init func(string), ccp ccprovider.ChaincodeProvider, sccp sysccprovider.SystemChaincodeProvider, pm txvalidator.PluginMapper) {
	...
	//NEW add
	fmt.Println("这里是core/peer/peer.go func Initialize()=============")
...
// NEW add
func GetLedgerForRollback(cid string) kvledger.RollbackInterface{
	chains.RLock()
	defer chains.RUnlock()
	if c, ok := chains.list[cid]; ok {
		return c.cs.rollbackInterface   // 这里不一样
	}
	return nil
}
