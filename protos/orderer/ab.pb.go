...
type AtomicBroadcast_BroadcastServer interface {
	...
	RecvM() (*common.CrossOverMsg, error) // NEW add
}
...
func (x *atomicBroadcastBroadcastServer) Recv() (*common.Envelope, error) {
	...
	fmt.Println("Orderer了！！这是fabric/protos/orderer/ab.pb.go Recv() m(common.Envelope)=", m)
	return m, nil
}

// NEW add
func (x *atomicBroadcastBroadcastServer) RecvM() (*common.CrossOverMsg, error) {
	m := new(common.CrossOverMsg) 
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
	fmt.Println("Orderer了！！这是fabric/protos/orderer/ab.pb.go RecvM() m(common.CrossOverMsg)=", m)	
	return m, nil
}
