...
//	fmt.Println("protos/utils/commonutils.go UnmarshalChannelHeader()")
	chdr := &cb.ChannelHeader{}
	err := proto.Unmarshal(bytes, chdr)
	if err != nil {
		return nil, fmt.Errorf("UnmarshalChannelHeader failed, err %s", err)
	}
//fmt.Println("UnmarshalChannelHeader() chdr = ", chdr)
	return chdr, nil
}
...
func ChannelHeader(env *cb.Envelope) (*cb.ChannelHeader, error) {
	fmt.Println("protos/utils/commonutils.go ChannelHeader()开始 ")
	envPayload, err := UnmarshalPayload(env.Payload)
	if err != nil {
		return nil, fmt.Errorf("payload unmarshaling error: %s", err)
	}
fmt.Println("protos/utils/commonutils.go ChannelHeader 标记1 envPayload = ", envPayload)

	if envPayload.Header == nil {
		return nil, fmt.Errorf("no header was set")
	}
	fmt.Println("protos/utils/commonutils.go ChannelHeader 标记2 envPayload.Header = ", envPayload.Header)

	if envPayload.Header.ChannelHeader == nil {
		return nil, fmt.Errorf("no channel header was set")
	}
	fmt.Println("protos/utils/commonutils.go ChannelHeader 标记3 envPayload.Header.ChannelHeader = ", envPayload.Header.ChannelHeader)
/*
	if string(env.CrossInfo) == "confirmation" { //NEW add
		chdrByte2Str := string(envPayload.Header.ChannelHeader)
		chidstr := chdrByte2Str[15:len(chdrByte2Str)-2] //{"channel_id":"wwqchannel"}
		fmt.Println("chdr.ChannelID截取", chidstr)
		chdr := &cb.ChannelHeader{
			ChannelId: chidstr,
		}
		return chdr, nil
...
*/
	...
	if err != nil {
		fmt.Println("protos/utils/commonutils.go ChannelHeader() err!=nil UnmarshalChannelHeader失败")
		return nil, fmt.Errorf("channel header unmarshaling error: %s", err)
	}
	fmt.Println("protos/utils/commonutils.go ChannelHeader()结束 chdr = ", chdr)
	return chdr, nil
}
