...
//NEW add
func GetPayloadFromBytes(p []byte) *common.Payload{
	payload := &common.Payload{}
	proto.Unmarshal(p, payload)
	return payload
}
		...
		if n == 0 {
			...
			if r.Response.Status < 200 || r.Response.Status >= 400 {
				if r.Response.Status == 401{
					fmt.Println("protos/utils/txutils.go CreateSignedTx() 401 r.Response.Status = ", r.Response.Status)  //NEW add
				}
...
//	return &peer.SignedProposal{ProposalBytes: propBytes, Signature: signature, CrossMsgBytes: }, nil
}
